...
async function loadAccounts() {
  try {
    const r = await window.storage.get("amara-accounts");
    return r && r.value ? JSON.parse(r.value) : {};
  } catch (e) { return {}; }
}
async function saveAccounts(obj) {
  try { await window.storage.set("amara-accounts", JSON.stringify(obj)); } catch (e) { console.error(e); }
}
...
export default function Amara() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("amara-session");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const r = await window.storage.get(`${ns}::events`);
        if (r && r.value) setEvents(JSON.parse(r.value));
      } catch (e) { /* none yet */ }
      setLoaded(true);
      try {
        const r2 = await window.storage.get(`${ns}::todos`);
        if (r2 && r2.value) setTodos(JSON.parse(r2.value));
      } catch (e) { /* none yet */ }
      try {
        const r3 = await window.storage.get(`${ns}::contacts`);
        if (r3 && r3.value) setContacts(JSON.parse(r3.value));
      } catch (e) { /* none yet */ }
      try {
        const r4 = await window.storage.get(`${ns}::templates`);
  ...
  const persist = useCallback(async (next) => {
    setEvents(next);
    if (!account) return;
    try { await window.storage.set(`${account.username}::events`, JSON.stringify(next)); } catch (e) { console.error(e); }
  }, [account]);

  const persistTodos = useCallback(async (next) => {
    setTodos(next);
    if (!account) return;
    try { await window.storage.set(`${account.username}::todos`, JSON.stringify(next)); } catch (e) { console.error(e); }
  }, [account]);

  const persistContacts = useCallback(async (next) => {
    setContacts(next);
    if (!account) return;
    try { await window.storage.set(`${account.username}::contacts`, JSON.stringify(next)); } catch (e) { console.error(e); }
  }, [account]);

  const persistTemplates = useCallback(async (next) => {
    setTemplates(next);
    if (!account) return;
    try { await window.storage.set(`${account.username}::templates`, JSON.stringify(next)); } catch (e) { console.error(e); }
  }, [account]);

  const persistDiary = useCallback(async (next) => {
    setDiary(next);
    if (!account) return;
    try { await window.storage.set(`${account.username}::diary`, JSON.stringify(next)); } catch (e) { console.error(e); }
  }, [account]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await window.storage.get(`${account.username}::diary`);
  ...
  useEffect(() => {
    ...
    (async () => {
      let last = null;
      try {
        const r = await window.storage.get(`${account.username}::lastBriefing`);
        last = r && r.value;
      } catch (e) { /* none yet */ }
      if (last !== todayISO()) {
        ...
        requestWeather();
        loadNewsBriefing();
        try { await window.storage.set(`${account.username}::lastBriefing`, todayISO()); } catch (e) { /* ignore */ }
      } else {
        requestWeather();
      }
    })();
  ...
  async function submitAuth() {
    ...
    try {
      ...
      if (authMode === "signup") {
        ...
        await window.storage.set("amara-session", username);
  ...
  async function logout() {
    try { await window.storage.delete("amara-session"); } catch (e) { /* ignore */ }
    setAccount(null);
    setSessionUser(null);
    setLoaded(false);
